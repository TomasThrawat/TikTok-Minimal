...
Email and password sign-up/sign-in use Supabase Auth with persistent sessions.
...
The feed uses AndroidX Media3 ExoPlayer with hardware-accelerated Android rendering, lifecycle-aware pause/resume, a bounded local cache, and next-item preloading.

Video uploads use the Supabase Kotlin resumable upload API with progress reporting. The Android app does not impose a small artificial video-size cap. Real service and plan limits still apply.
...
There are no fake production videos in the repository. A zero-video backend correctly shows an empty feed until users upload real content.
